...
### Don't promise more than the work item delivers

A title must describe what *this* work item completes, not the eventual benefit it contributes to. If
the value only lands once another team consumes the work, a user-facing title is a lie the board repeats
at every stand-up — and it makes the item impossible to call done.
...
| `Users can export to CSV` — but this item only adds the endpoint | `Add the CSV export endpoint` |

This usually travels with the type choice: if the title has to drop its user-facing framing to stay
honest, that's a strong hint the item is a task rather than a user-facing issue.
...
Then check the converse: *"When this work item is done, \_\_\_ is true."* If the only way to finish that
sentence involves work in someone else's item, the title is over-promising.
